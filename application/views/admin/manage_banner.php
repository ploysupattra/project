...
<html lang="en">
  ...
  <body>
      ...
      <div class="row" style="padding-top:20px;">
        <div class="container">
          <div class="row">
            <center><p class="titleh" style="font-size:55px;">จัดการลงโฆษณา</p></center>
          </div>
          <div class="row" >
            <a href="<?php echo base_url(); ?>conadmin/add_banner"><button class="btn btn-add nithan">เพิ่ม</button></a>
          </div>
        </div><br />
        <table class="table-fill">
          <thead>
            <tr><th class="text-left">ID</th>
                ...
                <th class="text-center nithan">ชื่อโฆษณา</th>
                <th class="text-center">Size</th>
                <th class="text-center nithan">วันที่เริ่ม</th>
                <th class="text-center nithan">วันที่ปิด</th>
                <th class="text-center nithan">แก้ไข</th>
                <th class="text-center nithan">ลบ</th></tr>
          </thead>
          <tbody class="table-hover">
            <?php foreach ($banner->result() as $row) { ?>
            <tr><td><?php echo $row->ban_id; ?></td>
                <td><img src="<?php echo base_url().'../assets/img/addbanner/'.$row->ban_pic; ?>" width="300" /></td>
                <td><?php echo $row->ban_name; ?></td>
                <td><?php echo $row->size_name; ?> <?php echo $row->size; ?></td>
                <td><?php echo $row->date_s; ?></td>
                <td><?php echo $row->date_e; ?></td>
                <td><form action="<?php echo base_url().'conadmin/udf_banner/'.$row->ban_id; ?>"><button class="btn btn-warning">Edit</button></form></td>
                <td><a href="<?php echo base_url().'conadmin/cfdl_banner/'.$row->ban_id;?>" class="btn btn-danger">Delete</a></td></tr>
            <?php } ?>
          </tbody>
        </table>
      </div>
