  ...
  <body>
      ...
      <div class="row" style="padding-top:100px;">
        <div class="container">
          ...
          <div class="col-md-8">
            <center><p class="titleh" style="font-size:55px;">เพิ่มโฆษณา</p><br /></center>
              <form action="<?php echo base_url(); ?>conadmin/addbanner" method="post" enctype="multipart/form-data">
                <div class="form-inline">
                  <div class="row">
                    <label>ชื่อโฆษณา : </label> <input class="form-control" type="text" name="banner_name"  />
                    <label>ตำแหน่งและขนาด :</label>
                      <select class="form-control" name="size">
                        <?php foreach ($sizeban->result() as $row) { ?>
                        <option value="<?php echo $row->size_id; ?>"><?php echo $row->size_name; ?> (<?php echo $row->size; ?>)</option>
                        <?php } ?>
                      </select>
                  </div><br />
                  <div class="row">
                    <label>วันที่เริ่มโฆษณา :</label> <input type="date" class="form-control"   name="start">
                    <label>วันที่ปิดโฆษณา :</label> <input type="date" class="form-control"  name="end">
                  </div><br />
                  <div class="row">
                    <label>เลือกรูป : </label> <input class="form-control" type="file" name="picture" style="padding-left:5px;">
                  </div><br />
                  <center><button type="submit" class="btn btn-success">เพิ่มโฆษณา</button></center>
                </div>
              </form>
          </div>
          <div class="col-md-2">
